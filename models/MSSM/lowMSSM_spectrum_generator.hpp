...
public:
   ...
   void run(const QedQcd& oneset, const lowMSSM_input_parameters<Two_scale>& input);
...
void lowMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                        const lowMSSM_input_parameters<Two_scale>& input)
{
   model.clear();
...
void lowMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   ...
   Coupling_monitor<lowMSSM<T>, MSSM_parameter_getter>
