// ====================================================================
// Test implementation of a separate spectrum generator that uses
// the semianalytic algorithm to solve the model
// ====================================================================

#ifndef CSE6SSM_SEMIANALYTIC_SPECTRUM_GENERATOR_H
#define CSE6SSM_SEMIANALYTIC_SPECTRUM_GENERATOR_H
...
template <class T>
class CSE6SSM_semianalytic_spectrum_generator {
public:
   CSE6SSM_semianalytic_spectrum_generator()
      : solver(), model()
      , constraint_handler()
      , high_scale(0.)
      , susy_scale(0.)
   ...
   const CSE6SSM_semianalytic<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const CSE6SSM_semianalytic_input_parameters<T>& input);
...
private:
   ...
   CSE6SSM_semianalytic<T> model;
   CSE6SSM_semianalytic_constraint_handler<T> constraint_handler;
 ...
 */
template <class T>
void CSE6SSM_semianalytic_spectrum_generator<T>::run(const QedQcd& oneset,
                                                     const CSE6SSM_semianalytic_input_parameters<T>& input)
{
   model.clear();
   ...
   CSE6SSM_susy_convergence_tester<T> inner_convergence_tester(&model, precision_goal);

   // convergence tester for the whole iteration
   CSE6SSM_semianalytic_convergence_tester<T> outer_convergence_tester(&model, precision_goal);
   ...
   CSE6SSM_semianalytic_initial_guesser<T> initial_guesser(constraint_handler.get_initial_guesser(&model, oneset));
 ...
 */
template <class T>
void CSE6SSM_semianalytic_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   CSE6SSM_semianalytic<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   CSE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<CSE6SSM_semianalytic<T>, CSE6SSM_parameter_getter>
 ...
 */
template <class T>
void CSE6SSM_semianalytic_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
