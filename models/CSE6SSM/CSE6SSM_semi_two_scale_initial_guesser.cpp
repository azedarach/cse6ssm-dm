...
CSE6SSM_semianalytic_initial_guesser<Two_scale>::CSE6SSM_semianalytic_initial_guesser(
   CSE6SSM_semianalytic<Two_scale>* model_,
   const QedQcd& oneset_,
   const CSE6SSM_semianalytic_low_scale_constraint<Two_scale>& low_constraint_,
   const CSE6SSM_semianalytic_susy_scale_constraint<Two_scale>& susy_constraint_,
   const CSE6SSM_semianalytic_high_scale_constraint<Two_scale>& high_constraint_,
   CSE6SSM_semianalytic_high_scale_constraint<Two_scale>* main_iteration_high_constraint_
)
...
{
   assert(model && "CSE6SSM_semianalytic_initial_guesser: Error: pointer to model"
          " CSE6SSM_semianalytic<Two_scale> must not be zero");
}
 ...
 */
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::guess()
{
 ...
 */
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::guess_susy_parameters()
{
   ...
   CSE6SSM_susy_convergence_tester<Two_scale> convergence_tester(model, running_precision);
...
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::guess_low_scale_parameters(CSE6SSM_semianalytic<Two_scale>* model, const QedQcd& oneset, double g1, double g2, double g3)
{
   const auto TanBeta = INPUTPARAMETER(TanBeta);
   const auto QSInput = INPUTPARAMETER(QSInput);
   const auto sInput = INPUTPARAMETER(sInput);
...
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::guess_high_scale_parameters()
{
...
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::calculate_DRbar_yukawa_couplings()
{
   calculate_Yu_DRbar();
   calculate_Yd_DRbar();
   calculate_Ye_DRbar();
 ...
 * corrections).
 */
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::calculate_Yu_DRbar()
{
   Eigen::Matrix<std::complex<double>,3,3> topDRbar(ZEROMATRIXCOMPLEX(3,3));
 ...
 */
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::calculate_Yd_DRbar()
{
   Eigen::Matrix<std::complex<double>,3,3> bottomDRbar(ZEROMATRIXCOMPLEX(3,3));
 ...
 * corrections).
 */
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::calculate_Ye_DRbar()
{
   Eigen::Matrix<std::complex<double>,3,3> electronDRbar(ZEROMATRIXCOMPLEX(3,3));
 ...
 * calculated.
 */
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::guess_soft_parameters()
{
   const double low_scale_guess = low_constraint.get_initial_scale_guess();
...
void CSE6SSM_semianalytic_initial_guesser<Two_scale>::Initial_guess_low_scale_constraint::set_model(Two_scale_model* model_)
{
